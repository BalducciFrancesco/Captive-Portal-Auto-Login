...
def get_browser(settings: Settings):
	...
	options.add_argument(f"--binary-location={settings.chrome_path}")
	...
	for attempt in range(settings.retries):
		try:
			...
			if attempt < settings.retries - 1:
				...
				log.error(f"Maximum number of attempts reached. Unable to initialize or navigate with Chrome driver: {e.msg}")
